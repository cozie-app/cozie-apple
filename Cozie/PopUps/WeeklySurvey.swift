    ...
    @IBAction func onClickSubmit(_ sender: UIButton) {
        if answers.count != questions.count {
            ...
            for (index, question) in questions.enumerated() {
                if multipleAnswer[index] == true {
                    var answers1 = ""
                    answers[index]?.forEach { option in
                        answers1 += answers1 == "" ? "" : ", "
                        if index == 1 && option == 6 {
                            answers1 += otherAnswer
                        } else {
...
extension WeeklySurvey: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
        ...
        return CGFloat(label.calculateMaxLines(forText: questions[section])) * 20
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.delegate = self
        buttons[indexPath.section]?.append(cell.button)
        cell.button.isSelected = answers[indexPath.section]?.contains(indexPath.row) == true
        cell.button.backgroundColor = cell.button.isSelected ? .lightGray : .systemBackground
        if indexPath.section == 1 && indexPath.row == 6 && answers[indexPath.section]?.contains(indexPath.row) == true {
            cell.otherTextField.isHidden = false
            cell.otherTextField.text = otherAnswer
        } else {
...
extension WeeklySurvey: selectAnswerDelegate {
    func onClickOption(cell: QuestionCell) {
        if let indexPath = tableQuestions.indexPath(for: cell) {
            ...
            if answers[indexPath.section]?.contains(indexPath.row) == true {
                cell.button.isSelected = false
                cell.button.backgroundColor = .systemBackground
                answers[indexPath.section]?.removeAll(where: { $0 == indexPath.row })
                if answers[indexPath.section]?.count == 0 {
                    answers.removeValue(forKey: indexPath.section)
                }
            } else {
                cell.button.isSelected = true
                cell.button.backgroundColor = .lightGray
                if multipleAnswer[indexPath.section] == true && answers[indexPath.section] != nil {
                    answers[indexPath.section]?.append(indexPath.row)
                } else {
                    answers[indexPath.section] = [indexPath.row]
                }
            }
